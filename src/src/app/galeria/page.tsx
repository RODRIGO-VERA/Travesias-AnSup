...
export default async function GaleriaPage() {
  ...
  const imagenes = panoramas.flatMap((p) => p.images.map((img) => ({ ...img, panorama: p.nombre })));
      ...
      <div className="columns-2 sm:columns-3 gap-4 [&>*]:mb-4">
        {imagenes.map((img, i) => (
          <div key={`${img.id}-${i}`} className="relative rounded-xl overflow-hidden break-inside-avoid">
            <Image src={img.url} alt={img.titulo || img.panorama} width={500} height={500} className="w-full h-auto object-cover" />
            <span className="absolute bottom-2 left-2 badge bg-deep-900/70 text-white">{img.panorama}</span>
          </div>
        ))}
      </div>
    </div>
